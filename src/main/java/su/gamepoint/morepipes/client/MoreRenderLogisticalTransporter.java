...
import su.gamepoint.morepipes.common.block.transmitter.logisticaltransporter.LTTier;
import su.gamepoint.morepipes.common.block.transmitter.logisticaltransporter.MoreTileEntityLogisticalTransporterBase;
import su.gamepoint.morepipes.common.block.transmitter.logisticaltransporter.MoreTransporterUtils;
...
public class MoreRenderLogisticalTransporter extends RenderTransmitterBase<MoreTileEntityLogisticalTransporterBase> {
    ...
    public MoreRenderLogisticalTransporter(BlockEntityRendererProvider.Context context) {
        super(context);
    ...
    @Override
    protected void render(MoreTileEntityLogisticalTransporterBase tile, float partialTick, PoseStack matrix, MultiBufferSource renderer, int light, int overlayLight,
        ...
        if (!inTransit.isEmpty()) {
            ...
            for (TransporterStack stack : reducedTransit) {
                float[] stackPos = MoreTransporterUtils.getStackPosition(transporter, stack, partial);
                matrix.pushPose();
